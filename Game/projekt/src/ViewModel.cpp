	...
	if (game->getPosUpdated())
	{
		...
		for (std::list<Object*>::const_iterator it = game->getObjectList().begin(); it != game->getObjectList().end(); ++it)
		{
				...
				((*it)->getHeight() + (*it)->getY())*GameDefinitions::scale > camera.y)
			{
                ...
                if (dynamic_cast<Door*>(*it))
                    SDL_SetRenderDrawColor(sdlWrapper->renderer, 0x99, 0x30, 0x00, 0x00);
                ...
                if (dynamic_cast<Chest*>(*it))
                SDL_SetRenderDrawColor(sdlWrapper->renderer, 0x80, 0x00, 0x00, 0x00);
                ...
                if (dynamic_cast<Bookshelf*>(*it))
                SDL_SetRenderDrawColor(sdlWrapper->renderer, 0x00, 0x80, 0x00, 0x00);
        ...
        SDL_Rect healthRect = { 30, 20, 20, 20 };
        SDL_RenderFillRect(sdlWrapper->renderer, &healthRect);
        SDL_SetRenderDrawColor(sdlWrapper->renderer, 0x0, 0x0, 0x0, 0xFF);
        SDL_RenderDrawRect(sdlWrapper->renderer, &healthRect);

        std::string heartRateString = std::to_string(dynamic_cast<PlayerCreature*>(game->getPlayerController()->getCreature())->getHeartRate());
        SDL_Texture* heartRateText = sdlWrapper->loadTextureFromRenderedText(heartRateString, { 0x0, 0x0, 0x0, 0xFF }, Font::RegularOutline);
        drawText(healthRect.x + 28, healthRect.y + (healthRect.h / 2) - 2, heartRateText, TextAlignment::Left);
        SDL_DestroyTexture(heartRateText);
        heartRateText = sdlWrapper->loadTextureFromRenderedText(heartRateString, { 0xFF, 0xFF, 0x0, 0xFF });
        drawText(healthRect.x + 30, healthRect.y + (healthRect.h / 2), heartRateText, TextAlignment::Left);
        SDL_DestroyTexture(heartRateText);
        ...
        std::string coinString = "x " + std::to_string(dynamic_cast<PlayerCreature*>(game->getPlayerController()->getCreature())->getKeys());
...
void ViewModel::drawWon()
{
    ...
    std::string endString = "You've found magical item and got out of SCARY HOUSE.";
    SDL_Texture* endText = sdlWrapper->loadTextureFromRenderedText(endString, { 0x0, 0x0, 0x0, 0xFF }, Font::Subtitle);
    drawText(10, 180, endText,TextAlignment::Left);
    SDL_DestroyTexture(endText);
    drawButton(rendererWidth / 2 - 60, rendererHeight - 100, 120, 25, sdlWrapper->menuTextureVector[MenuTexture::MenuButton], &Game::quitToMenu);
    SDL_RenderPresent(sdlWrapper->renderer);
#ifdef DEBUGGAME
    ++countedFrames;
