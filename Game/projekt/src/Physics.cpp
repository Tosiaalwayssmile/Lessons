 ...
 * Currently is changed to return constant 10
 */
...
void Physics::checkCollision(std::list<Object*> &objectList)
{
	...
	for (it = objectList.begin(); it != objectList.end(); ++it)
	{
        ...
        else if (tmpC->getY() > boundaryHeight - tmpC->getHeight())
        {
            tmpC->moveBy(0, -((tmpC->getHeight() + tmpC->getY()) - boundaryHeight));
            tmpC->addCollisionState(CollisionState::FromAbove);
        }
