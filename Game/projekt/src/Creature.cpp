 ...
 * @param width width of Creature.
 * @param height height of Creature.
 * @param heartRate heartRate of Creature.
 * @see SolidObject
 * @see health
 */
Creature::Creature(double x, double y, double width, double height, Uint8 heartRate) :
...
{
    SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "Creature created at x:%f, y:%f, with witdh:%f, height:%f and heartRate:%d", x, y, width, height, heartRate);
}
 ...
 * If damage equals 127 it ignores invulnerability.
 ...
 * @return void
 */
void Creature::scare(Sint8 damage)
{
    ...
    /**
     * Brings down heartRate of Creature.
     * If damage equals more than 80
     * @return void
     */
void Creature::coolDown()
{
    heartRate = 80;
}
...
void Creature::onCollision(SolidObject* collider)
{
    ...
    if (collider == nullptr)
	 ...
	 //here ignore the chest
    if (dynamic_cast<Chest*>(collider))
        return;
...
/**
 * Get current heartRate of Creature.
 * @return Uint8
 */
Uint8 Creature::getHeartRate() const
{
    return heartRate;
}
